package com.rustamsemenov.spring.security.configuration;
...
public class MySecurityConfig  extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(AuthenticationManagerBuilder auth) throws Exception {
        ...
        auth.inMemoryAuthentication().withUser(userBuilder.username("rustam").password("rustam").roles("EMPLOYEE"));
...
//        auth.jdbcAuthentication().dataSource(dataSource);
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.authorizeRequests().antMatchers("/").hasAnyRole("EMPLOYEE","HR", "MANAGER").
                antMatchers("/hr_info").hasRole("HR").antMatchers("/manager_info").hasRole("MANAGER").and().
                formLogin().permitAll();
    }
}
